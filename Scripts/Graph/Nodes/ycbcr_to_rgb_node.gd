...
func _ready():
	...
	add_child(hbox1)
	set_slot(1, true, universal_type, rgb_color, false, universal_type, Color.BLACK)
	
	# Second slot - Y (output only)
	...
	margin2.add_theme_constant_override("margin_right", 20)  # More space before the slot
	margin2.add_child(label2)
	
	hbox2.add_child(margin2)
	add_child(hbox2)
	set_slot(2, false, universal_type, rgb_color, false, universal_type, rgb_color)
	
	# Third slot - Cr (output only)
	...
	margin3.add_theme_constant_override("margin_right", 20)  # More space before the slot
	margin3.add_child(label3)
	
	hbox3.add_child(margin3)
	add_child(hbox3)
	set_slot(3, true, universal_type, cr_color, true, universal_type, rgb_color)
	
	# Fourth slot - Cb (output only)
	var hbox4 = HBoxContainer.new()
	hbox4.size_flags_horizontal = Control.SIZE_FILL
	hbox4.custom_minimum_size.y = 10
	hbox4.alignment = BoxContainer.ALIGNMENT_BEGIN  # Align to the right
	...
	set_slot(5, false, universal_type, cb_color, false, universal_type, Color.BLACK)
